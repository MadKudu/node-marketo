...
let hasReplay = false;
let createRequest = () =>
  marketo.bulkActivityExtract.create({
    createdAt: {
      startAt: moment.utc('2023-01-07'),
      endAt: moment.utc('2023-01-08'),
    },
  });
let cancelRequest = (res, done) =>
  marketo.bulkActivityExtract
  ...
  if (hasReplay)
    describe('#get', function () {
      it('coordinates an extract until completed', function (done) {
        this.timeout(60000 * 10);
        marketo.bulkActivityExtract
          .get({ createdAt: { startAt: moment(), endAt: moment() } })
          .then(function (response) {
            assert.equal(response.result.length, 1);
            assert.equal(response.result[0].status, 'Completed');
            done();
