...
describe('Retry logic', function () {
  ...
  it('does not retry on unauthorized, or quota reached', function (done) {
    ...
    connection
      ...
      .catch(function (err) {
        assert(
          errors.hasMarketoCode(err, errors.marketoErrorCodes.ACCESS_DENIED)
        );
      ...
      .catch(function (err) {
        assert(
          errors.hasMarketoCode(err, errors.marketoErrorCodes.QUOTA_REACHED)
        );
  ...
  it('tries to get a new oauth token when expired', function (done) {
    ...
    connection
      ...
      .catch(function (err) {
        assert(
          errors.hasMarketoCode(err, errors.marketoErrorCodes.TOKEN_EXPIRED)
        );
        assert.equal(server.count(path), 4);
  ...
  it('drops to max delay on rate limit error', function (done) {
    ...
    connection
      ...
      .catch(function (err) {
        assert(
          errors.hasMarketoCode(
            err,
            errors.marketoErrorCodes.RATE_LIMIT_REACHED
          )
        );
