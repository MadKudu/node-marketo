...
function getFirstMessage(data) {
  return data && data.errors && data.errors[0]
...
function isNetworkError(err) {
  var code = err.code || '';
  return (
    retryableNetworkErrorCodes.indexOf(code) > -1 || err instanceof TimeoutError
  );
}

function isServerError(err) {
  return (
    Math.floor(err.code / 100) === 5 ||
...
function isExpiredToken(err) {
  return (
    hasMarketoCode(err, mkCode.TOKEN_EXPIRED) ||
