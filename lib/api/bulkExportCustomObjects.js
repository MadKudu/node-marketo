...
BulkExportCustomObjects.prototype = {
  ...
  get: function (co_id, fields, filter, options) {
    ...
    return new Promise(function (resolve, reject) {
      return self
        .create(co_id, fields, filter, options)
        .then(function (data) {
          if (!data.success) {
            var msg = data.errors[0].message;
            log.error(msg);
          ...
          self
            ...
            .then(function (data) {
              ...
              self
                .statusTilCompleted(co_id, exportId)
                .then(resolve)
                .catch(reject);
            })
            .catch(function (err) {
              self
                .cancel(co_id, exportId)
                .then(function (data) {
                  return reject(err);
                })
                .catch(reject);
  ...
  fileStream: function (co_id, exportId, options) {
    return new MarketoStream(
      this.file(co_id, exportId, options).then(function (data) {
        return { result: [data] };
      })
    );
  },
